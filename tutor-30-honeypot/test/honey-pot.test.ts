...
describe("Honey", function () {
  async function deploy() {
    ...
    const bank: Bank = await BankFactory.deploy(logger.target);
    await bank.waitForDeployment();

    const AttackFactory = await ethers.getContractFactory("Attack");
    const attack: Attack = await AttackFactory.deploy(bank.target);
    ...
    const honeypotedBank: Bank = await BankFactory.deploy(honeypot.target);
    await honeypotedBank.waitForDeployment();

    const honeypotedAttack: Attack = await AttackFactory.deploy(honeypotedBank.target);
  ...
  it("attacks", async function () {
    const { bank, attack, attacker } = await loadFixture(deploy);
    ...
    const afterAttackBalance = ethers.formatEther(await bank.getBalance());
    expect(afterAttackBalance).to.eq("0.0");
  });

  it("honeypots hacker", async function () {
    const { honeypotedBank, honeypotedAttack, attacker } = await loadFixture(deploy);
    ...
    const currentBalance = ethers.formatEther(await honeypotedBank.getBalance());
    expect(currentBalance).to.eq(initialAmount);

    await expect(
      honeypotedAttack.connect(attacker).attack({ value: ethers.parseEther("1.0") })
    ).to.be.revertedWith("Failed to send Ether");
  });

  // it('honeypots regular users', async function() {
  //   const { honeypotedBank } = await loadFixture(deploy);
  ...
  it("doesnt honeypot regular users", async function () {
    const { honeypotedBank, deployer } = await loadFixture(deploy);
