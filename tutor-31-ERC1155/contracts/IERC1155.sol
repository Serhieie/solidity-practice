...
function setApprovalForAll(address operator, uint[] calldata ids) external view returns(uint[] memory);
function isApprovedForAll(address operant, address operator) external view returns(bool);
