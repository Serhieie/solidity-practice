...
abstract contract ERC1155 is IERC1155,  IERC1155MetadataURI {
    ...
    function balanceOf(
        address account,
         uint id) public view returns(uint){
    ...
    function balanceOfBatch(
        address[] calldata accounts,
         uint[] calldata ids
         ) public view returns(uint[] memory bachBalances){
        require(accounts.length == ids.length);

        bachBalances = new uint[](accounts.length);

        for(uint i = 0; i < accounts.length; i +=1){
            bachBalances[i]= balanceOf(accounts[i], ids[i]);
        }
    }
    ...
    function _doSafeTransferAcceptanceCheck(
        ...
        if(to.code.length > 0 ){
            ...
            operator, from, id, amount, data) returns(bytes4 resp){
    ...
    function _doSafeBatchTransferAcceptanceCheck(
        ...
        if(to.code.length > 0 ){
            ...
            operator, from, ids, amounts, data) returns(bytes4 resp){
